...
//		localClient();
	...
	private static void remoteClient() {
		...
		String resultPermit = client.target("http://"+ClientConfig.HOSTNAME+":"+ClientConfig.PORT).path("/data").queryParam("user", "alex")
		...
		String resultDeny = client.target("http://"+ClientConfig.HOSTNAME+":"+ClientConfig.PORT).path("/data").queryParam("user", "bob")
